...
class Corpora:
    ...
    stopwords: Set = field(default_factory=make_default)
    ...
    def metrics(self) -> List[Tuple[str, int]]:
        ...
        * Use count to return the requested amount of the top words, defaults to 5
        ...
        return Counter(x for x in self.cleaned.split() if x not in
                       self.stopwords).most_common(self.count)
    ...
    def graph(self) -> None:
        ...
        * The tag is repeated the number of counts of the word
        ...
        :return: None
        """
        for word, cnt in self.metrics:
            print(f'{word:>10} {self.tag * cnt}')
