...
def operas_both_at_premiere(guest, composer):
    ...
    def within_dates(start, date, end):
        return _get_date(start) <= _get_date(date) <= _get_date(end)
    ...
    for opera in composer_operas:
        if within_dates(_guest.born, opera.date, _guest.died) \
                and within_dates(_composer.born, opera.date, _composer.died):
            yield opera.play
