...
def player_with_max_points_per_game():
    ...
    cur.execute('''SELECT name FROM players WHERE CAST(avg_points as numeric)
                = (SELECT MAX(CAST(avg_points as numeric)) from players)''')
...
def year_with_most_drafts():
    ...
    cur.execute('''SELECT year, COUNT(year) as value_occurrence FROM players GROUP BY year ORDER BY value_occurrence DESC
    LIMIT    1;''')
    return cur.fetchone()[0]
